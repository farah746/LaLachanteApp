...
class ReservationclientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = DB::connection('mysql2')
            ->select('SELECT reservation.id_experience, Contact.nom, Contact.prenom, Contact.tel, Contact.email, Experience.statut_experience, reservation.num_facture, Factures.url_facture, reservation.id_contact, reservation.id_reservation 
                FROM Contact JOIN Experience JOIN Factures JOIN reservation
                ON reservation.num_facture = Factures.num_facture
                AND Experience.id_experience = reservation.id_experience
                AND Contact.id_contact = reservation.id_contact');
    ...
    public function show(Request $request)
    {
        ...
        $id_reserve = DB::connection('mysql2')->select('SELECT id_reservation FROM reservation');
        ...
        foreach ($id_reserve as $key => $value) {
            if ($url1 === $value->id_reservation) {
                $url = $url1;
            } elseif ($url2 === $value->id_reservation) {
                $url = $url2;
            } elseif ($url3 === $value->id_reservation) {
                $url = $url3;
            }
        }
        ...
        $dbs = DB::connection('mysql2')->select('SELECT Contact.nom, Contact.prenom, Contact.tel, Contact.email, Contact.adresse, Contact.code_postale, Contact.ville, Experience.nom_experience, reservation.num_facture, Factures.statut_facture, Factures.nb_chanson, Factures.nb_experimentateur, Factures.url_facture 
        FROM Contact JOIN Experience JOIN Factures JOIN reservation 
        ON reservation.num_facture = Factures.num_facture
        AND reservation.id_contact = Contact.id_contact
        AND Experience.id_experience = reservation.id_experience
        WHERE reservation.id_reservation=?', [$url]);
    ...
    public function valid(Request $request, Reservationclient $reservations)
    {
        $num_facture = $request->input('num_facture');
        $id_contact = $request->input('id_contact');

        return redirect()->away('https://automat.lalachante.fr:5678/webhook-test/331009fe-5af8-43ae-908c-cf56b4d9796d](https://automat.lalachante.fr:5678/webhook-test/331009fe-5af8-43ae-908c-cf56b4d9796d)?num_facture='. $num_facture .'& id_contact='. $id_contact);
    }
}
